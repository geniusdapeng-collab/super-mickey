...
每次预生产任务，**必须先进行需求清单确认**，流程如下：

1. 用户输入意图
2. 生成完整《视频需求要点清单》（含：视频类型、时长、风格、平台、目标受众、角色、创意指数、结构规划）
3. **等待用户确认**（或提出修改意见）
4. 确认后，锁定需求，进入预生产链路
...
**违规后果**：系统级错误，立即上报队长，重新跑完整流程。

**历史教训**：2026-06-20 因跳过需求清单确认，被队长严厉批评。已修复系统代码，移除 `skipRequirementConfirmation` 选项。
