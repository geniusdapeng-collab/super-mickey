...
// 警服角度定义（5个角度：正面、四分之三侧面、侧面、特写、全身）
const policeAngles = [
  {
    id: 'front',
    name: '警服-正面照',
  ...
  },
  {
    id: 'closeup',
    name: '警服-面部特写',
    prompt: `Professional female police officer portrait, close-up front view, tight crop on face, Chinese woman wearing standard dark navy blue police uniform cap visible, ${FACE_ANCHOR}, calm composed professional expression, sharp focus on facial features, photorealistic, high resolution, official documentation style`
  },
  {
    id: 'fullBody',
    name: '警服-全身像',
...
const lifeAngles = [
  {
    id: 'front',
    name: '生活-正面照',
    prompt: `Warm casual portrait photo, front view, head and shoulders, Chinese woman in comfortable home clothes, natural relaxed motherly smile, soft warm indoor lighting, cozy living room background blurred, ${FACE_ANCHOR}, warm approachable expression, photorealistic, high quality portrait, marketing-friendly mother character`
  },
  {
    id: 'threeQuarter',
    name: '生活-四分之三侧面',
    ...
    reference_images: [
      {
        ...
        reference_type: 'face'  // 尝试 face 而不是 character
      }
    ],
    size: '1920x1920',
    n: 1
...
async function main() {
  ...
  const report = {
    ...
    model: MODEL,
    policeResults: policeResults.map(r => ({
      angle: r.angle.id,
      name: r.angle.name,
      prompt: r.angle.prompt,
      ...r
    })),
    lifeResults: lifeResults.map(r => ({
      angle: r.angle.id,
      name: r.angle.name,
      prompt: r.angle.prompt,
      ...r
  ...
  fs.writeFileSync(
    path.join(OUTPUT_DIR, 'portraits-v2-report.json'),
