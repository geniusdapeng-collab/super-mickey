...
| 4个接入点（含片头后/storyboard后） | 香香彩虹桥架构不同（Layer 0-4） | **2个接入点**：Layer 2 后 + 最终导出前 |
| 14个模块 → 12个能力 | 香香彩虹桥模块较少 | 保持12个能力矩阵，与创意指数引擎联动 |

### ❌ 暂不采纳

| 建议 | 原因 |
|------|------|
| 批量修复历史输出脚本 | 香香彩虹桥历史输出较少，优先保障新链路 |
| `nirath-master-pipeline.js` 补丁 | 这是 v6.x 文件，香香彩虹桥使用 `index.js` |
