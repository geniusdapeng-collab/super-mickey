...
async function runPreproduction() {
  ...
  try {
    ...
    const resultPath = path.join(outputDir, `super-mickey-ep01-${LLM_MODE}-${timestamp}.json`);
    const reportPath = path.join(outputDir, `super-mickey-ep01-${LLM_MODE}-${timestamp}-report.md`);
    const promptsPath = path.join(outputDir, `super-mickey-ep01-${LLM_MODE}-${timestamp}-prompts.md`);
