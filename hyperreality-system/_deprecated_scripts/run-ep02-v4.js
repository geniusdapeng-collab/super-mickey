...
async function runPreproduction() {
  ...
  try {
    ...
    const resultPath = path.join(outputDir, `super-mickey-ep02-${timestamp}.json`);
    const reportPath = path.join(outputDir, `super-mickey-ep02-${timestamp}-report.md`);
    const promptsPath = path.join(outputDir, `super-mickey-ep02-${timestamp}-prompts.md`);
