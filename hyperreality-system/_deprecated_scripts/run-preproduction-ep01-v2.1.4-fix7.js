...
async function runPreproduction() {
  ...
  try {
    ...
    const resultPath = path.join(outputDir, `rainbow-bridge-ep01-${timestamp}.json`);
    const reportPath = path.join(outputDir, `rainbow-bridge-ep01-${timestamp}-report.md`);
    const promptsPath = path.join(outputDir, `rainbow-bridge-ep01-${timestamp}-prompts.md`);
