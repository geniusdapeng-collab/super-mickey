...
async function runPreproduction() {
  ...
  try {
    ...
    const resultPath = path.join(outputDir, `super-xiangbao-ep01-${timestamp}.json`);
    const reportPath = path.join(outputDir, `super-xiangbao-ep01-${timestamp}-report.md`);
    const promptsPath = path.join(outputDir, `super-xiangbao-ep01-${timestamp}-prompts.md`);
