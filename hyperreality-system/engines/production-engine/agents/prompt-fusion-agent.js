...
- 保持角色视觉锚点一致`;
  ...
  async process(shots, blueprint) {
    ...
    const TOTAL_AGENT_BUDGET = this.llmTimeout || 300000;
    const PER_SHOT_BUDGET = Math.floor(TOTAL_AGENT_BUDGET / Math.max(shots.length, 1));
    console.log(`[PromptFusionAgent] 每个镜头预算: ${PER_SHOT_BUDGET}ms`);
