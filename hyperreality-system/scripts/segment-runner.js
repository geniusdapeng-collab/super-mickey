...
const SEGMENTS = [
  {
    id: 'theme',
    name: 'Step 1: 清理 + 创意主题 + 需求清单',
    maxDuration: 5 * 60 * 1000, // 5分钟
    description: '清理旧数据，生成创意主题和需求清单',
    phases: ['cleanup', 'creativeTheme', 'requirementList']
  },
  {
    id: 'script',
    name: 'Step 2: 剧本生成',
    maxDuration: 10 * 60 * 1000, // 10分钟
  ...
  },
  {
    ...
    maxDuration: 20 * 60 * 1000, // 20分钟
    description: '融合提示词，分批处理镜头',
    phases: ['promptFusion'],
    batchSize: 2 // 每批处理2个镜头
  },
  {
    id: 'quality',
...
class SegmentRunner {
  ...
  /**
   * Segment 4: 提示词融合（分批处理）
   */
  async runFusionSegment() {
    const prevCheckpoint = this.loadCheckpoint('design');
    if (!prevCheckpoint) {
      throw new Error('Segment 4 需要 Segment 3 的 checkpoint，请先运行: node segment-runner.js design');
    }

    const { sceneDesign, visualLanguage, audioDesign } = prevCheckpoint.data;
    ...
    const allPrompts = [];
    const batchCount = Math.ceil(shots.length / batchSize);
    
    for (let i = 0; i < shots.length; i += batchSize) {
      const batch = shots.slice(i, i + batchSize);
      const batchNum = Math.floor(i / batchSize) + 1;
      
      this.heartbeat(`处理批次 ${batchNum}/${batchCount}`);
      console.log(`[Segment 4] 批次 ${batchNum}/${batchCount}: ${batch.map(s => s.shotId || s.id).join(', ')}`);
