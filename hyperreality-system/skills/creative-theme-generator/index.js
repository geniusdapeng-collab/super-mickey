...
class FieldCompleter {
  ...
  _generateDescription(type, theme, input) {
    const templates = {
      '医疗急救': `[时间+地点]，一名[患者状态]患者被推进抢救区。[核心动作]，[医疗设备]发出[声音/光效]，[环境氛围]。镜头聚焦[视觉焦点]，[情绪收束]。`,
      '硬科幻': `[时间+地点]，[角色]正在[关键动作]。突然[科幻事件]发生，[环境/空间变化]。[科技元素]开始运作，镜头展现[视觉奇观]，[悬念/思考]。`,
      '武侠动作': `[时间+地点]，[角色]手持[武器]。[动作描述]，[环境元素]随之[变化]。[对手]从[方向]出现，双方[对峙/交锋]，[结果/悬念]。`,
      '恐怖悬疑': `[时间+地点]，[角色]发现[异常现象]。[探索过程]，[恐怖元素]逐渐显现。[转折点]，[角色反应]，[开放式结局]。`,
      '自然纪录片': `[地点+时间]，[主体]正在[自然行为]。[环境细节]，[光影变化]。[特写镜头]展现[细节之美]，[升华/保护主题]。`,
      '浪漫爱情': `[时间+地点]，[角色A]与[角色B][关系状态]。[关键事件]发生，[情感变化]。[视觉焦点]在[细节]上停留，[情绪高潮/遗憾]。`,
      '家庭温情': `[时间+地点]，[家庭成员]正在[日常动作]。[细节描写]，[情感线索]。[转折/回忆]，[情感爆发点]，[温暖收束]。`,
      '喜剧荒诞': `[时间+地点]，[角色]遇到[荒诞情境]。[反应过程]，[升级事件]。[意外转折]，[荒诞高潮]，[讽刺/温暖结局]。`
    };
    const template = templates[type] || templates['硬科幻'];
    return template.replace(/\[.*?\]/g, (match) => {
      const options = {
        '[时间+地点]': ['深夜的城市天台', '清晨的海边', '黄昏的老街', '雨中的车站'],
        '[角色]': ['主角', '一个普通人', '孤独的灵魂'],
        '[关键动作]': ['做出重要决定', '面对内心恐惧', '寻找真相'],
        '[科幻事件]': ['时空扭曲', '外星信号', 'AI觉醒'],
        '[环境/空间变化]': ['重力消失', '建筑变形', '时间倒流'],
        '[视觉奇观]': ['星空倒悬', '城市折叠', '粒子重组'],
        '[悬念/思考]': ['留下一个开放式结局', '引发深层思考']
      };
      const key = match;
      const opts = options[key] || ['未知场景'];
