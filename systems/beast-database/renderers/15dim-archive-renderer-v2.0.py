...
# 标题
md_lines.append(f"# {data['name']['chinese']} — Nirath神兽深度档案")
md_lines.append(f"**档案编号**: {data['catalogNo']} | **神兽ID**: {data['id']}")
md_lines.append(f"**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
md_lines.append(f"**数据来源**: JSON唯一真相源 | **渲染引擎**: 15维度深度档案渲染器 v2.0")
md_lines.append("")
...
md_lines.append("永夜裂谷是Nirath星球北极圈一条绵延数千公里的地壳裂缝，深处是活跃的岩浆海洋。由于Nirath特殊的轴倾角和双恒星系统的光照盲区，北极常年处于绝对黑暗中，地表温度低至-180°C。唯有烛龙的存在为这片区域提供了光和热——其千里龙身散发的热量维持着裂谷周边-50°C至-20°C的相对温和区域，使得特定的荧光苔藓和耐热微生物得以生存。烛龙的光热辐射范围约500公里，形成一片独特的'温暖环带'，这是Nirath北极圈唯一的生命绿洲。")
md_lines.append("")
...
md_lines.append("①头部：人的面容，拥有完整的五官系统，可进行情感表达（如凝视、微笑、皱眉），这是烛龙区别于其他神兽的关键——它有人类的情感维度；②蛇身：长达千里，直径500米，无足而以腹部鳞片滑行，这种结构使其能在永夜裂谷的岩浆通道中自由穿行；③竖直双目：双眼竖直生长，开合决定光暗交替，这是其最核心的生物器官；④火精：口中衔持的恒星碎片，直径50米，是其能量核心；⑤鳞片系统：全身覆盖赤红色碳化硅-石墨烯鳞片，既是防护装甲，也是散热系统。")
md_lines.append("")
...
md_lines.append("①碳化硅-石墨烯复合鳞片：碳化硅提供超高硬度（莫氏硬度9.5，接近钻石），石墨烯提供导热性和柔韧性，这种复合结构使鳞片既能抵御永夜裂谷中-180°C的极寒和岩浆通道中1200°C的极热，又能保持足够的弹性以适应蛇身的弯曲；②微孔散热结构：鳞片表面的微孔直径约2毫米，可将火精核心产生的多余热量以红外辐射形式释放，防止过热；③腹部光滑鳞片：腹部鳞片表面粗糙度降低90%，减少滑行摩擦系数至0.05，使其能以100公里/小时的速度在岩壁上滑行。")
md_lines.append("")
...
md_lines.append("### 11.1 头部片段 (head)")
md_lines.append(f"```\n{pf['head']}\n```")
md_lines.append("")
md_lines.append("### 11.2 身体片段 (body)")
md_lines.append(f"```\n{pf['body']}\n```")
md_lines.append("")
md_lines.append("### 11.3 眼睛片段 (eyes)")
md_lines.append(f"```\n{pf['eyes']}\n```")
md_lines.append("")
md_lines.append("### 11.4 特殊片段 (special)")
md_lines.append(f"```\n{pf['special']}\n```")
md_lines.append("")
md_lines.append("**Prompt拼接指南**:")
md_lines.append("使用时应按 '场景描述 + head + body + eyes + special + 风格指令' 的顺序拼接，总字数控制在490字以内。如需突出某一特征，可将对应片段前置并增加细节。")
md_lines.append("")
...
md_lines.append("---")
md_lines.append("")
md_lines.append("**文档说明**: 本文档由JSON唯一真相源通过15维度深度档案渲染器 v2.0 自动生成，所有内容来源于 `/systems/beast-database/beasts/zhu-long.json`。")
md_lines.append("**文档结构**: 15个维度 = 命名体系(1) + 分类分级(2) + 原生状态(3) + 栖息地(4) + 外形描述(5) + 身体结构(6) + 体型规格(7) + 材质纹理(8) + 配色系统(9) + 标志性特征(10) + Prompt片段(11) + 能力矩阵(12) + 起源故事(13) + 关键传说(14) + 象征关系(15)")
md_lines.append("")
md_lines.append(f"**生成引擎**: Nirath神兽档案15维度渲染器 v2.0")
md_lines.append(f"**渲染时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

# 写入文件
output_path = '/root/.openclaw/workspace/systems/beast-database/archive-views/zhu-long-archive-v2.md'
os.makedirs(os.path.dirname(output_path), exist_ok=True)
with open(output_path, 'w') as f:
    f.write('\n'.join(md_lines))

print(f"✅ 烛龙15维度档案已生成: {output_path}")
