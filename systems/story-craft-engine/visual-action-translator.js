...
class VisualActionTranslator {
  ...
  static get CONCEPT_TO_ACTION_MAP() {
    return {
      ...
      '300年孤独': {
        ...
        visual: '饕餮脚下散落着数百个破碎的金属头盔/工具，AgentX是唯一站着的',
        verbs: ['散落', '站立', '靠近'],
        reaction: '满地残骸vsAgentX站立 = 历史与现在的视觉对比'
      },
      '长期守护': {
      ...
      '心跳快但不逃': {
        abstract: '心跳很快，但没有跑',
        visual: 'AgentX胸口微微起伏，手在颤抖但坚持向前伸出，一步没退',
        verbs: ['起伏', '颤抖', '伸出', '坚持'],
      ...
      '理解': {
        abstract: '理解比征服更有力量',
        visual: 'AgentX轻轻触碰饕餮的鼻子，饕餮闭上眼睛（信任），周围花海从地面蔓延绽放',
        verbs: ['触碰', '闭眼', '绽放'],
        reaction: '触碰=信任建立，闭眼=放下戒备，花海=环境回应情感'
      },
      '礼物': {
        abstract: '不是武器。是礼物。',
        visual: 'AgentX手掌摊开，一颗种子发出柔和的绿光；饕餮鼻子凑近，眼睛从暗红变琥珀色',
        verbs: ['摊开', '发光', '凑近', '变化'],
  ...
  forceVisualize(text, context) {
    ...
    const defaults = {
      ...
      deepen: `AgentX的倒影出现在一滩液态汞般的水面，${beastName || '异兽'}的瞳孔微微收缩（注意到入侵者）`,
      crack: `${beastName || '异兽'}做出了一个反常动作——它低下头，不是攻击，而是...嗅闻？`,
      twist: `特写：${beastName || '异兽'}的"可怕特征"开始发光/变化/展现出从未见过的温柔`,
      resonance: `${beastName || '异兽'}和AgentX同时看向同一个方向，环境中的某个元素（花/光/水）开始回应他们的存在`
    };
  ...
  static getRulesSummary() {
    return {
      ...
      examples: [
        ...
        { bad: '300年了，第一个不逃的', good: '满地破碎头盔中，AgentX是唯一站着的' },
        { bad: '我...不是怪物', good: '饕餮低头触碰枯萎的花，花瓣在它呼吸间重新绽放' },
        { bad: '谢谢...看见', good: 'AgentX触碰饕餮鼻子，饕餮闭眼，花海从地面蔓延绽放' }
      ]
    };
  }
